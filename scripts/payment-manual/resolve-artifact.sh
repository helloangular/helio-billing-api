...
resolve_from_nexus() {
  ...
  record="$(curl --silent --fail --user "$NEXUS_TOKEN" --get "$nexus_url/service/rest/v1/search/assets" \
    --data-urlencode "repository=$repository" --data-urlencode "name=payment-api-manual-test" \
    --data-urlencode "version=$release_version" \
    | jq -c '[.items[] | select(.path | endswith(".war"))] | .[0] // empty')"
  if [[ -z "$record" ]]; then
    echo "Nexus has no artifact for payment-api-manual-test $release_version in $repository" >&2
    return 1
  fi
  source="$(jq -r '.downloadUrl' <<<"$record")"
  expected="$(jq -r '.checksum.sha256 // empty' <<<"$record")"
  if [[ ! "$expected" =~ ^[0-9a-f]{64}$ ]]; then
    echo "Nexus recorded no sha256 for payment-api-manual-test $release_version" >&2
    return 1
  fi
  mkdir -p "$work_dir"
  package_path="$work_dir/payment-api-manual-test.war"
  ...
  echo "Resolved payment-api-manual-test $release_version from Nexus -> $digest" >&2
}
...
resolve_from_github() {
  ...
  package_path="$(find "$work_dir/unpacked" -type f -name 'payment-api-manual-test.war' | head -n 1)"
  if [[ -z "$package_path" ]]; then
    echo "Artifact $artifact_name does not contain payment-api-manual-test.war" >&2
    exit 3
  fi
  digest="sha256:$(shasum -a 256 "$package_path" | awk '{print $1}')"
