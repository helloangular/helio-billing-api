...
#
# Contexts: payment-api-manual-test (production, default), payment-api-manual-test-test,
# payment-api-manual-test-uat, payment-api-manual-test-preprod. All live on the same persistent
# on-premise Tomcat; each keeps its own serving-digest state file.
#
# Usage: scripts/deploy-tomcat.sh <sha256 digest> [candidate war]
# Env:   TOMCAT_CONTEXT (default payment-api-manual-test), TOMCAT_BASE_URL (default http://127.0.0.1:8080)
#        HELIO_TOMCAT_STATE_DIR (default ~/.helio-tomcat), CATALINA_HOME
set -euo pipefail

expected_digest="${1:?expected sha256 digest is required}"
candidate_war="${2:-}"
context="${TOMCAT_CONTEXT:-payment-api-manual-test}"
...
if [[ ! "$context" =~ ^payment-api-manual-test(-(test|uat|preprod))?$ ]]; then
