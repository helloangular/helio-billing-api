...
project_key="$(sed -n 's/^sonar.projectKey=//p' sonar-project-payment-manual.properties)"
...
jq -n --slurpfile input "$out/input.json" '{input: $input[0]}' \
  | curl --fail --silent --show-error -X POST -H 'Content-Type: application/json' \
      --data-binary @- "$opa_url/v1/data/helio/payment_manual/decision" \
  > "$out/decision.json"

allow="$(jq -r '.result.allow // false' "$out/decision.json")"
echo "Policy helio.payment_manual for release $release_id -> $target_env: allow=$allow"
