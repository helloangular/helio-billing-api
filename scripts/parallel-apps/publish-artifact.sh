...
#   io/helio/$APP_SLUG/<version>/$APP_SLUG-<version>.war
#
...
registry_sha256() {
  curl --silent --fail --connect-timeout 5 --max-time 30 --user "$NEXUS_TOKEN" --get "$nexus_url/service/rest/v1/search/assets" \
    --data-urlencode "repository=$repository" --data-urlencode "name=$artifact" \
    --data-urlencode "version=$version" \
    | jq -er --arg path "$group_path/$version/$artifact-$version.war" '
      if .continuationToken != null then error("Unexpected paginated registry result") else . end |
      [.items[] | select(.path == $path)] |
      if length == 0 then "" elif length == 1 then .[0].checksum.sha256
      else error("Expected exactly one artifact") end'
}
...
if [[ -n "$existing" ]]; then
  ...
  http_code="$(curl --silent --show-error --connect-timeout 5 --max-time 120 --user "$NEXUS_TOKEN" --upload-file "$war" \
