...
#
# Contexts: $APP_SLUG (production, default) and $APP_SLUG-test.
# Both live on the same persistent
# on-premise Tomcat; each keeps its own serving-digest state file.
#
# Usage: scripts/deploy-tomcat.sh <sha256 digest> [candidate war]
# Env:   TOMCAT_CONTEXT (default $APP_SLUG), TOMCAT_BASE_URL (default http://127.0.0.1:8080)
...
candidate_war="${2:-}"
context="${TOMCAT_CONTEXT:-$APP_SLUG}"
...
artifact_store="$state_root/artifacts/$APP_SLUG"
...
if [[ "$context" != "$APP_SLUG" && "$context" != "$APP_SLUG-test" ]]; then
