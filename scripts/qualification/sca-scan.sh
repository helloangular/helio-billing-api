...
bash "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/sca-db-prepare.sh"
echo "Trivy $(trivy --version | head -n 1 | sed 's/Version: //')"
trivy fs --quiet --timeout 5m --skip-db-update --format cyclonedx --output "$out/sbom.cdx.json" .
trivy fs --quiet --timeout 5m --skip-db-update --scanners vuln,secret --ignore-unfixed \
